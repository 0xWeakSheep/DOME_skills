...
/**
 * Sanitize position data object
 */
export function sanitizePositionData<T extends Record<string, unknown>>(data: T): T {
  const sanitized = { ...data };

  // Sanitize string fields that may contain user-generated content
  const stringFields = ['title', 'label'];
  for (const field of stringFields) {
    if (typeof sanitized[field] === 'string') {
      (sanitized as Record<string, unknown>)[field] = sanitizeString(
        sanitized[field] as string,
