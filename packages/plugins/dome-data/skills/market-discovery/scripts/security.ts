 ...
 */
export function sanitizeStringArray(items: string[] | null | undefined, maxLength: number = 100): string[] {
...
export function sanitizeMarketData<T extends Record<string, unknown>>(data: T): T {
  ...
  // Sanitize tags array
  if (Array.isArray((sanitized as Record<string, unknown>).tags)) {
    (sanitized as Record<string, unknown>).tags = sanitizeStringArray((sanitized as Record<string, unknown>).tags as string[], 50);
  }
 ...
 */
export function secureProcess<T>(
  data: T,
  processor: (data: T) => T
): T {
  try {
