...
export async function fetchActivity(
  ...
  return makeRequest("/polymarket/activity", queryParams, apiKey) as Promise<
    ActivityResponse
  >;
}
 ...
 */
export function filterMergesSplitsRedeems(
  activities: Activity[],
 ...
 */
export function trackLargeTransactions(
  activities: Activity[],
...
export function analyzeActivityPatterns(
  ...
  // Basic statistics
  const totalShares = activities.reduce(
    (sum, a) => sum + a.shares_normalized,
    0
  );
  const avgShares = totalShares / activities.length;

  // Type distribution
  const typeDistribution: Record<ActivityType, number> = {
    MERGE: 0,
    SPLIT: 0,
    REDEEM: 0,
  };
    ...
    if (currentCluster.length === 0) {
      ...
      if (activity.timestamp - lastActivity.timestamp <= clusterWindow) {
        ...
        if (currentCluster.length >= 2) {
          const clusterShares = currentCluster.reduce(
            (sum, a) => sum + a.shares_normalized,
            0
          );
  ...
  const sortedByShares = [...activities].sort(
    (a, b) => b.shares_normalized - a.shares_normalized
  );
  ...
  if (largeVolumeActivities.length > 0) {
    const avgLargeShares =
      largeVolumeActivities.reduce((sum, a) => sum + a.shares_normalized, 0) /
  ...
  const userStats: Record<
    string,
    { count: number; total_shares: number }
  > = {};
  for (const activity of activities) {
...
export function detectMarketClosingSignals(
  activities: Activity[]
): MarketClosingSignal[] {
  ...
  const signals: MarketClosingSignal[] = [];
  ...
  for (const [market_slug, stats] of Object.entries(marketStats)) {
    ...
    const redeemRatio =
      stats.redeem_count + stats.other_count > 0
